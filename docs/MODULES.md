...
- **API Controllers**: `AuthController` (root — `GET api/auth/me` returns `UserId`/`EmployeeId`/`UserName`/`IsAuthenticated` plus the caller's effective `Roles`/`Permissions` via `GetEffectivePermissionsQuery`), and `Controllers/Identity/`: `PermissionCatalogController`, `PermissionController`, `RoleController`, `RolePermissionsController`, `UserController`, `UserRolesController`.
